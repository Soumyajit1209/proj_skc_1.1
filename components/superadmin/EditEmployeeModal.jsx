...
const EditEmployeeModal = ({ show, setShow, formData, setFormData, error, setError, submitting, setSubmitting, branches, selectedEmployee, setSelectedEmployee, fetchEmployees, fetchStats, token }) => {
  if (!show || !selectedEmployee) return null;

  const handleEditEmployee = async () => {
    if (!formData.employee_name.trim() || !formData.employee_phone.trim() || !formData.employee_username.trim()) {
      setError('Full name, phone number, and username are required');
      return;
    }
    setSubmitting(true);
    try {
      ...
      if (formData.employee_password) formDataToSend.append('password', formData.employee_password);
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/superadmin/employees/${selectedEmployee.emp_id}`, {
        method: 'PUT',
      ...
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <label htmlFor="employee_password" className="block text-xs font-medium text-gray-700 mb-1">Password (Optional)</label>
              <input
                type="password"
            ...
            <div>
              <label htmlFor="is_active" className="block text-xs font-medium text-gray-700 mb-1">Status</label>
              <select
                id="is_active"
                name="is_active"
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  disabled={submitting}
                  className="flex-1 px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:border-green-500"
                />
