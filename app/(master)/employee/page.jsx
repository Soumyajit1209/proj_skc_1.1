...
const EmployeeManagement = () => {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/all-employees`,
        {
          headers: {
  ...
  const handleUpdateEmployee = async (updatedEmployee) => {
    ...
    if (!user?.branch_id) {
      toast.error("Branch information missing. Please contact support.", { toastId: "missing-branch-id-update" });
      return;
    }

    try {
      const formData = new FormData();
      formData.append("emp_id", updatedEmployee.get("emp_id"));
      formData.append("full_name", updatedEmployee.get("full_name"));
      formData.append("phone_no", updatedEmployee.get("phone_no"));
      formData.append("email_id", updatedEmployee.get("email_id"));
      formData.append("aadhaar_no", updatedEmployee.get("aadhaar_no"));
      formData.append("username", updatedEmployee.get("username"));
      formData.append("is_active", updatedEmployee.get("is_active"));
      formData.append("branch_id", user.branch_id); // Ensure branch_id is sent
      if (updatedEmployee.get("profile_picture")) {
        formData.append("profile_picture", updatedEmployee.get("profile_picture"));
      }
  ...
  const handleAddEmployee = async (newEmployee) => {
    ...
    try {
      ...
      formData.append("branch_id", newEmployee.branch_id); // Already fixed
  ...
  const handleDeleteEmployee = async (emp_id) => {
    ...
    if (!user?.branch_id) {
      toast.error("Branch information missing. Please contact support.", { toastId: "missing-branch-id-delete" });
      return;
    }

    try {
      console.log("Deleting employee ID:", emp_id, "Admin branch_id:", user.branch_id); // Debug log
      ...
      if (response.status === 404) {
        toast.error("Employee not found.", { toastId: "delete-employee-404" });
        return;
      }
    ...
    <div className="flex items-center gap-2 mb-4">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${
            statusFilter === "inactive" ? "bg-red-100 text-red-800" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
          }`}
        >
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full">
          ...
          <p className="text-gray-600 mb-6">
            Are you sure you want to delete {employee?.full_name || "this employee"}? This action cannot be undone.
          </p>
